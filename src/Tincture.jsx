...
// ============================================================================
// BOTTLE MODAL — final stage. Records how many bottles produced, the bottle
// size, optional total yield in fl oz (for analytics), and a date.
// ============================================================================
function BottleModal({ batch, hobbyId, update, onClose }) {
  const [bottledDate, setBottledDate] = useState(batch.bottledDate || todayIso());
  const [bottlesMade, setBottlesMade] = useState(
    batch.bottlesMade ? String(batch.bottlesMade) : ""
  );
  const [bottleSize, setBottleSize] = useState(batch.bottleSize || "1oz");
  ...
  return (
    <Modal open onClose={onClose} title={`Bottle — ${batch.name}`}>
      ...
      <Field label="Total yield in fl oz (optional)">
        <input type="number" min={0} step="0.1" inputMode="decimal" style={inputStyle} value={totalYieldOz} onChange={e => setTotalYieldOz(e.target.value)} placeholder="e.g. 12" />
        <div style={{ fontSize: 11, color: palette.inkSoft, marginTop: 4, lineHeight: 1.4 }}>
          For analytics. Helps track how much you recovered per batch.
        </div>
      </Field>
