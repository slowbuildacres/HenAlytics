    ...
    <div style={{ marginBottom: 14, padding: 12, background: palette.card, border: `1.5px solid ${palette.line}`, borderRadius: 10 }}>
      ...
      {photos.length === 0 && (
        <div style={{ fontSize: 12, color: palette.inkSoft, marginBottom: 8, lineHeight: 1.5 }}>
          Add a profile photo, then keep adding over time — the journal shows them as a timeline so you can see how this cat has grown.
        </div>
      )}
...
function DogModal({ cat, cats, onSave, onDelete, onClose, update, user, hobbyId }) {
  ...
  return (
    <ModalShell title={cat ? "Edit cat" : "Add a cat"} onClose={onClose}>
      ...
      {cat && update && (
        <AnimalPhotoSection animal={cat} hobbyId={hobbyId} update={update} user={user} />
      )}
    </ModalShell>
  );
}
...
// ----------------------------------------------------------------------------
// Lets users record a simple per-sex head count for cats they don't want to
      ...
      <div style={{ background: palette.card, border: `1.5px solid ${palette.line}`, borderRadius: 12, padding: 14, marginBottom: 12 }}>
        ...
        {total > 0 ? (
          ...
          <div style={{ fontSize: 12, color: palette.inkSoft, marginTop: 6, lineHeight: 1.5 }}>
            Got cats you don't want to name individually? Record a simple head count by sex here — separate from your named animals.
          </div>
        )}
      </div>
      {open && (
        <Modal open onClose={() => setOpen(false)} title="Herd tally">
          <div style={{ fontSize: 13, color: palette.inkSoft, marginBottom: 14, lineHeight: 1.5 }}>
            Record a head count by sex for cats you don't want to track individually. Separate from your named animals — it doesn't affect breeding, history, or sales.
          </div>
...
export default function CatsPage({ hobby, data, update, setModal, user }) {
