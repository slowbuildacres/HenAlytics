...
// ----------------------------------------------------------------------------
// Lets users record a simple per-sex head count for rabbits they don't want to
...
function HerdTallySection({ hobby, update }) {
  const HERD_TALLY_CATEGORIES = ["Doe","Buck","Unsexed"];
      ...
      <div style={{ background: palette.card, border: `1.5px solid ${palette.line}`, borderRadius: 12, padding: 14, marginBottom: 12 }}>
        ...
        {total > 0 ? (
          ...
          <div style={{ fontSize: 12, color: palette.inkSoft, marginTop: 6, lineHeight: 1.5 }}>
            Got rabbits you don't want to name individually? Record a simple head count by sex here — separate from your named animals.
          </div>
        )}
      </div>
      {open && (
        <Modal open onClose={() => setOpen(false)} title="Herd tally">
          <div style={{ fontSize: 13, color: palette.inkSoft, marginBottom: 14, lineHeight: 1.5 }}>
            Record a head count by sex for rabbits you don't want to track individually. Separate from your named animals — it doesn't affect breeding, history, or sales.
          </div>
...
export default function RabbitsPage({hobby,data,update,user}){
  ...
  return(
    <div>
      <RabbitsModalRouter modal={localModal} hobby={hobby} update={update} user={user} onClose={()=>setLocalModal(null)}/>
